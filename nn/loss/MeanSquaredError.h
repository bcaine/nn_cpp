...
public:
     ...
     */
    Eigen::Tensor<Dtype, Dims> backward(const Eigen::Tensor<Dtype, Dims> &predictions, const Eigen::Tensor<Dtype, Dims> &labels);
};

template <typename Dtype, int Dims>
Dtype MeanSquaredError<Dtype, Dims>::loss(const Eigen::Tensor<Dtype, Dims> &predictions,
                                          const Eigen::Tensor<Dtype, Dims> &labels) {
    assert(predictions.dimensions()[0] == labels.dimensions()[0] && "MeanSquaredError::loss dimensions don't match");
    assert(predictions.dimensions()[1] == labels.dimensions()[1] && "MeanSquaredError::loss dimensions don't match");
